...
export const getUserPosts = async (req, res) => {
  const { _id } = req.params;
  try {
    const posts = await Posts.find({ createdUser: _id })
      .populate("createdUser")
      .populate("currentMember");
    res.status(200).json(posts);
  } catch (error) {
...
export const applyPost = async (req, res) => {
  ...
  try {
    ...
    if (
      curPost.currentMember.length + 2 <= curPost.maxMember &&
      !curPost.currentMember.some((member) => member.toString() === userID)
    ) {
      const newPost = await Posts.findByIdAndUpdate(
        _id,
        { $push: { currentMember: userID } },
        { new: true }
      )
        .populate("createdUser")
        .populate("currentMember");
