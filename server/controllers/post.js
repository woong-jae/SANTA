...
export const applyPost = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send("No post with that _id");
  try {
    const newPost = await Posts.findOneAndUpdate({ _id: _id, $expr: { $lt : ["$currentMemberLength", "$maxMember"]}, currentMember: { $nin: userID }, appliedMember: { $nin: userID }}, { $push: { appliedMember: userID } }, { new: true }).populate("createdUser").populate("currentMember");
    res.json(newPost);
  } catch (error) {
    res.status(404).json({ message: error });
  }
...
export const acceptMember = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send("No post with that _id");
  try {
    const newPost = await Posts.findOneAndUpdate({ _id: _id, $expr: { $lt : ["$currentMemberLength", "$maxMember"]}, currentMember: { $nin: userID }, appliedMember: { $in: userID }}, { $push: { currentMember: userID }, $inc: { currentMemberLength: 1 }, $pull: { appliedMember: userID} }, { new: true }).populate("createdUser").populate("currentMember");
    res.json(newPost);
  } catch (error) {
    res.status(404).json({ message: error });
  }
